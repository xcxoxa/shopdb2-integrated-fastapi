...
router = APIRouter(
    prefix="/api/system",
    tags=["조원 2 확장 (버전/배포/백업/기능제어)"],
)


@router.get(
    "/versions",
    response_model=List[schemas.ApplicationVersionResponse],
    summary="앱 버전 목록 조회",
)
...
@router.get(
    "/deployments",
    response_model=List[schemas.DeploymentHistoryResponse],
    summary="배포 이력 조회",
)
...
@router.get(
    "/backups",
    response_model=List[schemas.DatabaseBackupHistoryResponse],
    summary="DB 백업 이력 조회",
)
...
@router.get(
    "/rollbacks",
    response_model=List[schemas.RollbackHistoryResponse],
    summary="롤백 이력 조회",
)
...
@router.get(
    "/feature-flags",
    response_model=List[schemas.FeatureFlagResponse],
    summary="피처 플래그 목록 조회",
)
